...
const PricingForm = ({ prevForm, nextForm, range, setRange, deadline, setDeadline }) => {
    const MIN = 5, MAX = 2000, STEP = 5;

    const fix = () => {
        if (parseInt(range[1]) < parseInt(range[0])) setRange([range[0], range[0]]);
    }
            ...
            <div className='range-values'>
                ...
                <div>
                    <input type="number" value={range[0]}
                        ...
                        onBlur={(e) => { e.target.value = range[0].toString(); fix(); }}
                    />
                    to
                    <input type="number" value={range[1]}
                        onChange={(e) => changeRange([range[0], e.target.value])}
                        onFocus={(e) => { e.target.value = ''; }}
                        onBlur={(e) => { e.target.value = range[1].toString(); fix(); }}
                    />
                </div>
            </div>
